...
import com.covercorp.kaelaevent.minigame.games.tower.arena.task.TowerStartingTask;
...
public final class TowerMatchListener implements Listener {
    private final TowerArena arena;

    public TowerMatchListener(final TowerArena arena) {
        this.arena = arena;
    }
    ...
    public void onInteract(final PlayerInteractEvent event) {
        ...
        if (accessor.equals("start_game")) {
            if (arena.getState() != TowerMatchState.WAITING) {
                sender.sendMessage(arena.getTowerMiniGame().getMiniMessage().deserialize("<red>The game is already started!"));
                return;
            }

            sender.sendMessage(StringUtils.translate("&7[Tower] Starting game..."));
            arena.getAnnouncer().sendGlobalMessage("&6" + sender.getName() + "&7" + " is trying to START the game.", false);

            if (arena.getTowerMiniGame().getPlayerHelper().getPlayerList().size() < 2) {
            ...
            if (!arena.getTowerMatchProperties().isStarting()) {
                arena.getTowerMatchProperties().setStarting(true);
                arena.getTowerMatchProperties().setStartingTaskId(Bukkit.getScheduler().runTaskTimer(arena.getTowerMiniGame().getKaelaEvent(), new TowerStartingTask(arena), 0L, 20L).getTaskId());
            ...
            Bukkit.getScheduler().cancelTask(arena.getTowerMatchProperties().getStartingTaskId());

            arena.getTowerMatchProperties().resetTimer();

            arena.setState(TowerMatchState.WAITING);
        ...
        if (accessor.equals("stop_game")) {
            ...
            sender.sendMessage(StringUtils.translate("&7[Tower] Stopping game..."));
    ...
    public void onItemDrop(final PlayerDropItemEvent event) {
        ...
        final Optional<TowerPlayer> towerPlayerOptional = arena.getTowerMiniGame().getPlayerHelper().getPlayer(player.getUniqueId())
                .map(miniGamePlayer -> (TowerPlayer) miniGamePlayer);

        if (towerPlayerOptional.isEmpty()) return;

        if (arena.getState() != TowerMatchState.WAITING) {
            player.sendMessage(arena.getTowerMiniGame().getMiniMessage().deserialize("<red>You can't drop items while playing!"));
            event.setCancelled(true);
        }
