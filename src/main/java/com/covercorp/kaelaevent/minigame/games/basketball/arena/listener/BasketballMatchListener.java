...
import com.covercorp.kaelaevent.minigame.games.basketball.arena.task.BasketballStartingTask;
...
public final class BasketballMatchListener implements Listener {
    private final BasketballArena arena;

    public BasketballMatchListener(final BasketballArena arena) {
        this.arena = arena;
    }
    ...
    public void onInteract(final PlayerInteractEvent event) {
        ...
        if (accessor.equals("start_game")) {
            if (arena.getState() != BasketballMatchState.WAITING) {
                sender.sendMessage(arena.getBasketballMiniGame().getMiniMessage().deserialize("<red>The game is already started!"));
                return;
            }

            sender.sendMessage(StringUtils.translate("&7[Basketball] Starting game..."));
            arena.getAnnouncer().sendGlobalMessage("&6" + sender.getName() + "&7" + " is trying to START the game.", false);

            if (arena.getBasketballMiniGame().getPlayerHelper().getPlayerList().size() < 2) {
            ...
            if (!arena.getBasketballMatchProperties().isStarting()) {
                arena.getBasketballMatchProperties().setStarting(true);
                arena.getBasketballMatchProperties().setStartingTaskId(Bukkit.getScheduler().runTaskTimer(arena.getBasketballMiniGame().getKaelaEvent(), new BasketballStartingTask(arena), 0L, 20L).getTaskId());
            ...
            Bukkit.getScheduler().cancelTask(arena.getBasketballMatchProperties().getStartingTaskId());

            arena.getBasketballMatchProperties().resetTimer();

            arena.setState(BasketballMatchState.WAITING);

            sender.sendMessage(StringUtils.translate("&7[!] Stopped match start."));
            sender.playSound(sender, Sound.UI_BUTTON_CLICK, 0.8F, 0.8F);
        }
        if (accessor.equals("stop_game")) {
            if (arena.getState() != BasketballMatchState.GAME) {
                sender.sendMessage(StringUtils.translate("&cThe game is not started! If the game is paused, you must resume it first!"));
                return;
            }

            sender.sendMessage(StringUtils.translate("&7[Basketball] Stopping game..."));
    ...
    public void onItemDrop(final PlayerDropItemEvent event) {
        ...
        final Optional<BasketballPlayer> tugPlayerOptional = arena.getBasketballMiniGame().getPlayerHelper().getPlayer(player.getUniqueId())
                .map(miniGamePlayer -> (BasketballPlayer) miniGamePlayer);

        if (tugPlayerOptional.isEmpty()) return;

        if (arena.getState() != BasketballMatchState.WAITING) {
            player.sendMessage(arena.getBasketballMiniGame().getMiniMessage().deserialize("<red>You can't drop items while playing!"));
            event.setCancelled(true);
        }
