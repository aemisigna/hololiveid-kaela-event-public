...
import com.covercorp.kaelaevent.minigame.games.snowball.arena.task.SnowballStartingTask;
...
public final class SnowballMatchListener implements Listener {
    private final SnowballArena arena;

    public SnowballMatchListener(final SnowballArena arena) {
        this.arena = arena;
    }
    ...
    public void onInteract(final PlayerInteractEvent event) {
        ...
        if (accessor.equals("start_game")) {
            if (arena.getState() != SnowballMatchState.WAITING) {
                sender.sendMessage(arena.getSnowballMiniGame().getMiniMessage().deserialize("<red>The game is already started!"));
                return;
            }

            sender.sendMessage(StringUtils.translate("&7[Snowball] Starting game..."));
            arena.getAnnouncer().sendGlobalMessage("&6" + sender.getName() + "&7" + " is trying to START the game.", false);

            if (arena.getSnowballMiniGame().getPlayerHelper().getPlayerList().size() < 2) {
            ...
            if (!arena.getSnowballMatchProperties().isStarting()) {
                arena.getSnowballMatchProperties().setStarting(true);
                arena.getSnowballMatchProperties().setStartingTaskId(Bukkit.getScheduler().runTaskTimer(arena.getSnowballMiniGame().getKaelaEvent(), new SnowballStartingTask(arena), 0L, 20L).getTaskId());
            ...
            Bukkit.getScheduler().cancelTask(arena.getSnowballMatchProperties().getStartingTaskId());

            arena.getSnowballMatchProperties().resetTimer();

            arena.setState(SnowballMatchState.WAITING);

            sender.sendMessage(StringUtils.translate("&7[!] Stopped match start."));
            sender.playSound(sender, Sound.UI_BUTTON_CLICK, 0.8F, 0.8F);
        }
        if (accessor.equals("stop_game")) {
            if (arena.getState() != SnowballMatchState.GAME) {
                sender.sendMessage(StringUtils.translate("&cThe game is not started! If the game is paused, you must resume it first!"));
                return;
            }

            sender.sendMessage(StringUtils.translate("&7[Snowball] Stopping game..."));
    ...
    public void onItemDrop(final PlayerDropItemEvent event) {
        ...
        final Optional<SnowballPlayer> snowballPlayerOptional = arena.getSnowballMiniGame().getPlayerHelper().getPlayer(player.getUniqueId())
                .map(miniGamePlayer -> (SnowballPlayer) miniGamePlayer);

        if (snowballPlayerOptional.isEmpty()) return;

        if (arena.getState() != SnowballMatchState.WAITING) {
            player.sendMessage(arena.getSnowballMiniGame().getMiniMessage().deserialize("<red>You can't drop items while playing!"));
            event.setCancelled(true);
        }
