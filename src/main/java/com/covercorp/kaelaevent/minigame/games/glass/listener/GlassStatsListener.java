...
public final class GlassStatsListener implements Listener {
    private final GlassMiniGame miniGame;

    private final GlassArena arena;

    public GlassStatsListener(final GlassMiniGame miniGame) {
    ...
    public void onPlayerDamage(final EntityDamageEvent event) {
        ...
        if (arena.getState() == GlassMatchState.WAITING) {
            event.setCancelled(true);
            return;
        }

        final Optional<GlassPlayer> gamePlayerOptional = arena.getPlayerHelper().getPlayer(player.getUniqueId())
